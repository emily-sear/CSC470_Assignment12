  ...
  (lambda (no-code)
    (cond
      ...
      ((eq? (car no-code) 'block)
       (list 'blocky-exp (map no-parser (cdr no-code))))
...
(define run-blocky-expression
  (lambda (blocky-expression env)
    (cond
      ((null? blocky-expression) '())
      ((eq? (caar blocky-expression) 'remember-exp) (run-blocky-expression (cdr blocky-expression) (run-parsed-code (car blocky-expression) env)))
      (else (cons (run-parsed-code (car blocky-expression) env) (run-blocky-expression (cdr blocky-expression) env))))))
...
(define run-parsed-code
  (lambda (parsed-no-code env)
    (cond
      ...
      ((eq? (car parsed-no-code) 'blocky-exp)
       (run-blocky-expression (cadr parsed-no-code) env))
...
;(pop-to-global env)
(define sample-no-code '(block (remember a 15) (remember b 17) (spit-out a) (spit-out b) (remember c 50) (spit-out x)))
;sample-no-code
